...
import { searchEntities, getEntities } from './file.reducer';
  ...
  return (
    <div>
      <h2 id="file-heading" data-cy="FileHeading">
        <Translate contentKey="ta3LimApp.file.home.title">Files</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="ta3LimApp.file.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="ta3LimApp.file.home.createLabel">Create new File</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('ta3LimApp.file.home.search')}
                />
        ...
        {fileList && fileList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="ta3LimApp.file.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('label')}>
                  <Translate contentKey="ta3LimApp.file.label">Label</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('file')}>
                  <Translate contentKey="ta3LimApp.file.file">File</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="ta3LimApp.file.resource">Resource</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {fileList.map((file, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    {file.file ? (
                      <div>
                        {file.fileContentType ? (
                          <a onClick={openFile(file.fileContentType, file.file)}>
                  ...
                  <td>{file.resource ? <Link to={`resource/${file.resource.id}`}>{file.resource.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="ta3LimApp.file.home.notFound">No Files found</Translate>
            </div>
          )
        )}
      </div>
